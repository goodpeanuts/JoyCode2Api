...
```bash
./JoyCodeProxy serve --skip-validation
```
...
`./JoyCodeProxy serve` 默认是前台进程，关掉终端就停。长期运行的方式：

```bash
# 1. nohup（最简单）
nohup ./JoyCodeProxy serve --skip-validation > joycode.log 2>&1 &

# 2. 守护进程模式（崩溃自动重启）
./JoyCodeProxy daemon --skip-validation

# 3. 装成系统服务（macOS launchd / Linux systemd）
./JoyCodeProxy service install
./JoyCodeProxy service status
./JoyCodeProxy service uninstall
```
...
docker run -d -p 34891:34891 --name joycode-proxy joycode-proxy serve --skip-validation
```
...
```bash
./JoyCodeProxy service install      # 安装（macOS launchd / Linux systemd）
./JoyCodeProxy service status       # 查看状态
./JoyCodeProxy service uninstall    # 卸载
```
...
```
cmd/JoyCodeProxy/        CLI 入口 + HTTP 服务器
...
你直接执行了 `./JoyCodeProxy`，但**没加 `serve` 子命令**——根命令只会打印帮助然后退出（exit 0）。正确启动：
...
首次访问 <http://localhost:34891> 会进入初始化页面，让你设置 root 密码（≥ 6 位）。这是 Dashboard 登录密码，跟 JoyCode 账号无关。忘了密码可以用 `./JoyCodeProxy reset-password` 重置。
</details>
...
换端口：`./JoyCodeProxy serve -p 34892`。
</details>

<details>
<summary><b>关闭终端后服务就停了</b></summary>

`serve` 是前台进程。用 `nohup ./JoyCodeProxy serve --skip-validation > joycode.log 2>&1 &` 后台跑，或装成系统服务 `./JoyCodeProxy service install`。
</details>
